...
ggplot(exam, aes(x = ENGLISH, y = CLASS)) +
  geom_density_ridges(fill = "gray", color = "black", scale = 1.5) +
  theme_minimal() +
  labs(x = "ENGLISH", y = "CLASS")
...
::: callout-important
Including `calc_ecdf = TRUE` in `stat_density_ridges()` is important when you're working with discrete or binned data or when your dataset is small or skewed, because it ensures that the density estimate respects the actual distribution of your data by basing it on the empirical cumulative distribution function (ECDF).

-   **Preserves data distribution accuracy**:
    -   By default, `stat_density_ridges()` uses kernel density estimation (KDE), which **smooths** the distribution.

    -   If your data is **not continuous** or has **spikes or gaps**, KDE might misrepresent it.

    -   `calc_ecdf = TRUE` uses ECDF to **better capture irregularities** in real data (especially in test scores like English grades that often cluster at certain values).
-   **Reduces risk of artificial modes or tails**: - KDE may **create modes** (peaks) that don’t exist in your raw data.

```         
-    ECDF-based density avoids this by anchoring the density more directly to actual data points.
```

<!-- -->

-   **Improves reliability for smaller samples**:

```         
-    In small datasets, KDE often over-smooths.

-    ECDF helps retain the **discrete nature** and **true spread** of limited data.
```
:::
...
[**ggstatsplot**](https://indrajeetpatil.github.io/ggstatsplot/index.html) ![](https://r4va.netlify.app/chap10/img/image1.jpg){width="43"} is an extension of [**ggplot2**](https://ggplot2.tidyverse.org/) package for creating graphics with details from statistical tests included in the information-rich plots themselves.
...
exam %>%
  ggplot(aes(x = RACE, y = MATHS)) +
  stat_pointinterval(
    aes(color = after_stat(factor(.width))),   # Add color inside stat
    .width = c(0.95, 0.99)
  ) +
  labs(
    title = "Math Scores by Race with 95% and 99% Confidence Intervals",
